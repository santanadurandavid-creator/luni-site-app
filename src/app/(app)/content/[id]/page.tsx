...
export default function ContentItemPage() {
    ...
    useEffect(() => {
        if (!isLoading) {
            if (id) {
                if (isAuthenticated) {
                    // User is authenticated, redirect to content page with open parameter
                    router.replace(`/content?open=${id}`);
                } else {
                    // User is not authenticated, redirect to access-content page
                    router.replace(`/access-content?redirect=${encodeURIComponent(`/content?open=${id}`)}`);
                }
            }
        }
