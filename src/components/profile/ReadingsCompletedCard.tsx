...
export function ReadingsCompletedCard() {
    const { user, resetReadingsCompleted } = useAuth();
    const { t } = useLanguage();
    ...
    const handleConfirmReset = async () => {
        await resetReadingsCompleted();
        setShowConfirm(false);
    };

    const readingsCompleted = user.readingsCompleted || 0;
        ...
        <>
            ...
            <AlertDialog open={showConfirm} onOpenChange={setShowConfirm}>
                <AlertDialogContent className="rounded-2xl max-w-[85%] sm:max-w-md border-border bg-card">
                    <AlertDialogHeader>
                        <AlertDialogTitle className="font-headline text-lg">¿Reiniciar Contador de Lecturas?</AlertDialogTitle>
                        <AlertDialogDescription>
                            Esta acción pondrá tu contador de lecturas completadas a cero.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
